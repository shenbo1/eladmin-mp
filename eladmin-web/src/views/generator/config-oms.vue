          ...
          <el-form size="small" label-width="90px">
            <el-table v-loading="loading" :data="data" :max-height="tableHeight" size="small" style="width: 100%;margin-bottom: 15px">
              ...
              <el-table-column label="关联字典">
                <template slot-scope="scope">
                  <el-select v-model="data[scope.$index].dictName" filterable class="edit-input" clearable size="mini" placeholder="请选择" allow-create>
      ...
      <el-col>
        <el-card class="box-card" shadow="never">
          ...
          <el-form ref="form" :model="form" size="small" label-width="78px">
            <el-form-item label="类名称" prop="author">
              <el-input v-model="form.author" style="width: 40%" />
              <span style="color: #ff0000;margin-left: 10px;">类名称</span>
            </el-form-item>
            <el-form-item label="路由前缀" prop="pack">
              <el-input v-model="form.pack" style="width: 40%" />
              <span style="color: #ff0000;margin-left: 10px;">route.config.ts path</span>
            </el-form-item>
            <el-form-item label="模块文件夹名称" prop="moduleName">
              <el-input v-model="form.moduleName" style="width: 40%" />
              <span style="color: #ff0000;margin-left: 10px;">新建的文件夹名称-也是在路由上的名称</span>
            </el-form-item>
            ...
            <el-form-item label="接口名称" prop="apiAlias">
              <el-select v-model="form.apiAlias" class="edit-input" style="width: 40%" filterable placeholder="请选择">
                <el-option v-for="item in dict.api_config" :key="item.id" :label="item.label" :value="item.value" />
              </el-select>
              <!-- <el-select v-model="form.apiAlias" filterable class="edit-input" clearable size="mini" placeholder="请选择" allow-create>
                  <el-option  key="work-order" label="工单" value="work-order" />
             </el-select> -->
              <!-- <el-input v-model="form.apiAlias" style="width: 40%" /> -->
              <!-- <span style="color: #C0C0C0;margin-left: 10px;">接口的名称，用于控制器与接口文档中</span> -->
              <span style="color: #ff0000;margin-left: 10px;">选择api地址</span>
            </el-form-item>
...
import { save, sync, generatorOMS } from '@/api/generator/generator'
  ...
  methods: {
    ...
    toGen() {
      ...
      save(this.data).then(res => {
        this.notify('保存成功', 'success')
        // 生成代码
        generatorOMS(this.tableName, 0).then(data => {
