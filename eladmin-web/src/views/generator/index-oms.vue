      ...
      <el-table-column label="操作" width="160px" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button size="mini" style="margin-right: 2px" type="text">
            <router-link :to="'/sys-tools/generator/preview-oms/' + scope.row.tableName">
              预览
            </router-link>
          </el-button>
          ...
          <el-button size="mini" style="margin-left: -1px;margin-right: 2px" type="text">
            <router-link :to="'/sys-tools/generator/config-oms/' + scope.row.tableName">
              配置
            </router-link>
          </el-button>
...
import { generatorOMS, sync } from '@/api/generator/generator'
...
export default {
  ...
  methods: {
    toGen(tableName) {
      // 生成代码
      generatorOMS(tableName, 0).then(data => {
        this.$notify({
          title: '生成成功',
          type: 'success',
          duration: 2500
    ...
    toDownload(tableName) {
      // 打包下载
      generatorOMS(tableName, 2).then(data => {
